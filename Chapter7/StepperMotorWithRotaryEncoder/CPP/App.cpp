...
void RunRotary(StepperMotorController& steppper,const OKFPGAModule& device);
...
void RunTopLevel(const OKFPGAModule& device)
{
    ...
    std::cout << "ROTARY - Use the rotary encoder" << std::endl;
    std::cout << "QUIT - exit this application" << std::endl;

    auto stepper=StepperMotorController::Instance(device,true);
...
void RunRotary(StepperMotorController& stepper,const OKFPGAModule& device)
{
    std::cout << "Turn the rotary encoder, when you stop the stepper motor will emulate" << std::endl;
    stepper.SetSpeed(90);

    auto moveStart=false;
    short lastMove=0;
    auto moveCounts=0;
    while(true)
    {
        ...
        moveCounts=0;
        lastMove=0;
        const int moveCountStable=50;
        while(true)
        {
            ::Sleep(10);
            const short movement=device.GetWireOut(0x01,true);
            
            if(!moveStart)
            {
                if(lastMove!=movement) moveStart=true;
            }
            else
            {
            ...
            if(moveCounts==moveCountStable) break;
        }
        
        std::cout << "Last commanded move = " << lastMove << " counts       " << "\n";
        ...
        const double pulsesPerRevolution=24;
        const double edgesPerRevolution=pulsesPerRevolution*4;
        const double degreesPerRevolution=360.0;
        const double degreesPerEdge=degreesPerRevolution/edgesPerRevolution;
        const unsigned short degrees=abs(lastMove*degreesPerEdge);
        stepper.Move(degrees);
        device.ActivateTriggerIn(0x00,1);
        while(stepper.IsMoving()) { ::Sleep(20); }
    }
}
